...
const Store = {
  ...
  checkDevMode() {
    const isDev =
      new URLSearchParams(window.location.search).get("dev") !== null || !window.chrome || !window.chrome.webview
  ...
  loadFromLocalStorage() {
    try {
      ...
      if (savedSnippets) {
        ...
        console.info("从本地存储加载片段数据成功")
      } else {
        console.info("本地存储中没有片段数据，使用默认值")
        this.snippetsData = JSON.parse(JSON.stringify(this.defaultSnippets))
      ...
      if (savedSettings) {
        ...
        console.info("从本地存储加载设置数据成功")
      } else {
        console.info("本地存储中没有设置数据，使用默认值")
        this.settings = JSON.parse(JSON.stringify(this.defaultSettings))
